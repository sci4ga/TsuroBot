...
class Servo(object):
	...
	def tilt_absolute(self, tilt):
		...
		target_pulse = (pulse_max - pulse_min )/200 * (tilt + 100) + 500
		if target_pulse > self.pulse_max: target_pulse = self.pulse_max
		if target_pulse < self.pulse_min: target_pulse = self.pulse_min
	...
	def pan_absolute(self, pan):
		...
		if -100 > tilt > 100:
			return None
		target_pulse = (pulse_max - pulse_min )/200 * (pan + 100) + 500
		if target_pulse > self.pulse_max: target_pulse = self.pulse_max
		if target_pulse < self.pulse_min: target_pulse = self.pulse_min
	...
	def tilt_relative(self, tilt):
		...
		if -100 > tilt > 100:
			return None
		target_pulse = (pulse_max - pulse_min )/200 * (pan + 100) + 500
		offset_pulse = (pulse_max - pulse_min )/200 * (self.v_offset ) + 500
	...
	def pan_relative(self, pan):
		...
		if -100 > tilt > 100:
			return None
		target_pulse = (pulse_max - pulse_min )/200 * (pan + 100) + 500
		offset_pulse = (pulse_max - pulse_min )/200 * (self.h_offset ) + 500
		target_pulse = target_pulse + offset_pulse
		if target_pulse > self.pulse_max: target_pulse = self.pulse_max
		if target_pulse < self.pulse_min: target_pulse = self.pulse_min
		while(self.HPulse != target_pulse):
